...
async function main() {
  ...
  // routed through the guarded byreal() wrapper (no separate shell string here)
  const parsed = await byreal(`positions close --nft-mint ${row.pos.nftMintAddress} --auto-swap --output-mint ${usdc} --unsigned-tx --wallet-address ${cfg.ownerWallet}`);
